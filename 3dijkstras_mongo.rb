    ...
    while pq.size != 0
        ...
        if edges.size > 0
            edges.each do |edge|
                ...
                weight = edge[:weight]
                if !visited[destination_node_id] and (shortest_distances[destination_node_id] || INFINITY) > shortest_distances[current_node_id] + weight
                    shortest_distances[destination_node_id] = shortest_distances[current_node_id] + weight
                    previous[destination_node_id] = current_node_id
                    pq.push(destination_node_id)
                end
            end
        end
    end
